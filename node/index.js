...
app.get('/api/transactions', function (request, response, next) {
  Promise.resolve()
    .then(async function () {
      ...
      for(let i = 0; i<recently_added.length; i++){
        const data = {...recently_added[i]};
        if(data.transaction_id == null || data.transaction_id === undefined){
          continue;
        }

        await collectionRef.add(data);

        // const query = collectionRef.where("transaction_id", '==', data.transaction_id);
...
app.get('/api/balance', function (request, response, next) {
  Promise.resolve()
    .then(async function () {
      ...
      const collectionRef = db.collection("balance");
      
      collectionRef.add(balanceResponse.data)
        .then((docRef) => {
...
const prettyPrintResponse = (response) => {
  response.data && console.log(util.inspect(response.data, { colors: true, depth: 4 }));
};
